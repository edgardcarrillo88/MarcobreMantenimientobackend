...
//Metalurgia
const MetalurgiaModel = require("../models/Metalurgia/Inputs");
const MetalurgiaCalibracionModel = require("../models/Metalurgia/Calibracion");
...
const registerpolines = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Datos de polines registrados correctamente",
    });
  } catch (error) {
    console.error("Error al registrar datos de polines:", error);
    res
      .status(500)
      .json({ success: false, message: "Error al registrar datos de polines" });
  }
};
...
const getpolinesregisterdata = async (req, res) => {
  ...
  try {
    ...
    const cursor = polinesreportmodel
      .find({})
      .select(
        "Tag Ubicacion Bastidor Posicion Fecha Estado Descripcion TipoReporte Usuario"
      )
      .lean()
...
const GetPOlinesReportStream = async (req, res) => {
  ...
  try {
    ...
    while (true) {
      const data = await polinesreportmodel
        .find({})
        .select(
          "Tag Ubicacion Bastidor Posicion Fecha Estado Descripcion TipoReporte Usuario"
        )
...
const pruebacronologica = (req, res) => {
  ...
  const job = schedule.scheduleJob(rule, async function () {
    console.log(
      "Función programada para ejecutarse todos los días a las 00:01 UTC-5"
    );
    prueba(req, res);
  });
};
  ...
  try {
    ...
    const [coincidenciasDelDia, coincidenciasAyer] = await Promise.all([
      polinesreportmodel.find({
        ...
        $or: BasePolines.map(({ Tag, Ubicacion, Bastidor, Posicion }) => ({
          Tag,
          Ubicacion,
          Bastidor,
          Posicion,
    ...
    await Promise.all(
      BasePolines.map(async (docN1) => {
        const { Tag, Ubicacion, Bastidor, Posicion } = docN1;

        // Procesamiento de coincidencias del día actual
        const CoincidenciaDelDia = coincidenciasDelDia.filter(
          (coincidencia) =>
            coincidencia.Tag === Tag &&
            coincidencia.Ubicacion === Ubicacion &&
            coincidencia.Bastidor === Bastidor &&
            coincidencia.Posicion === Posicion
        );

        // Procesamiento de coincidencias del día anterior
        const coincidenciaAnterior = coincidenciasAyer.filter(
          (coincidencia) =>
            coincidencia.Tag === Tag &&
            coincidencia.Ubicacion === Ubicacion &&
            coincidencia.Bastidor === Bastidor &&
        ...
        if (
          CoincidenciaDelDia.length === 0 &&
          coincidenciaAnterior.length !== 0
        ) {
          const nuevosObjetos = coincidenciaAnterior
            .map((Objeto) => {
              // Verificar si el objeto ya ha sido creado anteriormente
              const objetoRepetido = objetosCreados.find(
                (obj) =>
                  obj.Tag === Objeto.Tag &&
                  obj.Ubicacion === Objeto.Ubicacion &&
                  obj.Bastidor === Objeto.Bastidor &&
                  obj.Posicion === Objeto.Posicion &&
          ...
          if (nuevosObjetos.length > 0) {
            ...
            console.log(
              "Se crearon nuevos objetos en la Base de Datos de reporte de polines:",
              nuevosObjetos
            );
          } else {
          ...
          console.log(
            "No se encontraron coincidencias en la Base de Datos de reporte de polines para el día anterior."
          );
        }
      })
    );
...
const borrandoDatosAutomaticos = async (req, res) => {
  console.log(
    "Borrando todos los polines con TipoReporte igual a 'Automático'"
  );
  try {
    await polinesreportmodel.deleteMany({ TipoReporte: "Automático" });
    console.log(
      'Todos los polines con TipoReporte igual a "Automático" han sido eliminados correctamente'
    );
    res
      .status(200)
      .send(
...
const CambioPolines = async (req, res) => {
  ...
  try {
    for (const data of req.body) {
      ...
      const dataPrincipal = await polinestagmodel.findByIdAndUpdate(
        polinesData._id,
        {
          $set: {
            Estado: "Cambio",
...
const uploadexceliw37nbase = (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      console.log(
        "Todos los datos del Iw37nBase guardados en la base de datos"
      );
      res.status(200).json({ message: "Datos guardados en la base de datos" });
    })
    .catch((error) => {
...
const uploadexceliw37nreport = (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      console.log(
        "Todos los datos guardados de IW37nReport en la base de datos"
      );
      res.status(200).json({ message: "Datos guardados en la base de datos" });
    })
    .catch((error) => {
...
const uploadexceliw39report = (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      console.log(
        "Todos los datos de iw39report guardados en la base de datos"
      );
      res.status(200).json({ message: "Datos guardados en la base de datos" });
    })
    .catch((error) => {
...
const uploadexceliw29report = (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      console.log(
        "Todos los datos de iw29report guardados en la base de datos"
      );
      res.status(200).json({ message: "Datos guardados en la base de datos" });
    })
    .catch((error) => {
...
const deleteallIw37nreport = async (req, res) => {
  ...
  iw37nreportmodel
    ...
    .then(() => {
      ...
      res
        .status(200)
        .send("Todos los datos de IW37n Report eliminados correctamente");
    })
    .catch((error) => {
      console.error("Error al eliminar documentos:", error);
    });
...
const getalldataIW39Report = async (req, res) => {
  ...
  try {
    ...
    const cursor = iw39reportmodel.find({}).lean().cursor();
...
const getalldataIW29Report = async (req, res) => {
  ...
  try {
    ...
    const cursor = iw29reportmodel.find({}).lean().cursor();
...
const getalldataIW47Report = async (req, res) => {
  ...
  try {
    ...
    const cursor = iw47reportmodel.find({}).lean().cursor();
...
const updatetempReportIndicadores = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      Message: "Oki doki de indicadores iw47reportmodel",
      semana: numeroDeSemana,
    });
    console.log("Objetos actualizados");
  } catch (error) {
    res.status(500).json({
      Message: "Error al actualizar los indicadores",
      error: error.message,
    });
  }
};
...
const TemporalEliminarSemana = async (req, res) => {
  ...
  try {
    ...
    const response2 = await iw37nreportmodel.deleteMany({
      Semana: "46",
      Anho: "2025",
    });
    const response3 = await iw39reportmodel.deleteMany({
      Semana: "46",
      Anho: "2025",
    });
    const response4 = await iw29reportmodel.deleteMany({
      Semana: "46",
      Anho: "2025",
    });
    const response5 = await iw47reportmodel.deleteMany({
      Semana: "46",
      Anho: "2025",
    });
    ...
    res
      .status(200)
      .send("Todos los datos iw39reportmodel eliminados correctamente");
  } catch (error) {
    console.log(error);
  }
};
...
const PruebaIW29 = async (req, res) => {
  ...
  const data = await iw29reportmodel.find({ Semana: "49" });
  ...
  const data2 = await iw29reportmodel.deleteMany({ Semana: "49" });
};
...
const uploadexcelPersonalContratistas = (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      console.log(
        "Todos los datos guardados de Personal contratista en la base de datos"
      );
      res.status(200).json({ message: "Datos guardados en la base de datos" });
    })
    .catch((error) => {
...
const EvaluacionPdP = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Datos guardados de manera satisfactoria" });
...
const GuardarDatosPreAvisos = async (req, res) => {
  ...
  try {
    ...
    const data = new Backlogreportmodel(req.body);
  ...
  try {
    ...
    const updateData = {
      ...
      ToggleServicioEspecializado:
        updateFields.ToggleServicioEspecializado === "true",
...
const RegistrarAndamios = async (req, res) => {
  ...
  console.log(req.body);
  try {
    const data = new AndamiosReporte(req.body);

    const datahistorial = new AndamiosReporteHistorial(req.body);
...
const ActualizarAndamios = async (req, res) => {
  ...
  try {
    const data = await AndamiosReporte.findByIdAndUpdate(
      _id,
      {
        $set: {
          Planta,
...
const LoadProvisionesTemp = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Datos procesados correctamente", datos: excelData });
  } catch (error) {
...
const UpdateDataCompromisos = async (req, res) => {
  ...
  Promise.all(dataPromises)
    .then(() => {
      ...
      res
        .status(200)
        .json({ message: "Datos Actualizados en la base de datos" });
    })
    .catch((error) => {
      console.error("Error al guardar los datos:", error);
      res.status(500).json({ error: "Error al guardar los datos" });
...
const GetAllMaterial = async (req, res) => {
  ...
  try {
    const GetAll = async () => {
      const [IH09, MB52, MCBA, ME2L, ME5A1, ME5A2, ZMM003JP11, ZMM003JP14] =
        await Promise.all([
          IH09Model.find({}),
          MB52Model.find({}),
          MCBAModel.find({}),
...
const CrearInspeccion = async (req, res) => {
  ...
  try {
    ...
    const data = new inspeccionesmodel(req.body);
...
const CargarDatosMetalurgia = async (req, res) => {
  console.log("Cargando datos de metalurgia");
  ...
  const dataPromises = excelData.map(async (rowData) => {
    try {
      console.log("cargando datos");
      const data = new MetalurgiaModel(rowData);
      await data.save();
    } catch (error) {
      console.error("Error al guardar el dato:", error);
    }
...
const DeleteInputsMetalurgia = async (req, res) => {
  console.log("Borrando datos de inputs de metalurgia");

  try {
    const response = await MetalurgiaModel.deleteMany({});
    res.status(200).json({ Message: "Ok" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ Message: error });
...
const CargarDatosCalibracionMetalurgia = async (req, res) => {
  console.log("Cargando datos de calibración de metalurgia");
  const bufferData = req.file.buffer;
  const workbook = xlsx.read(bufferData, { type: "buffer" });
  const worksheet = workbook.Sheets[workbook.SheetNames[0]];
  const excelData = xlsx.utils.sheet_to_json(worksheet);
  ...
  const dataPromises = excelData.map(async (rowData) => {
    try {
      console.log(rowData.date_hour);
      const fechainicio = new Date((rowData.fecha - 25569) * 86400 * 1000); //Convierte el número Excel a "días desde 1970 (25569 días)".

      rowData.fecha = fechainicio.toISOString();

      rowData.date_hour = rowData.date_hour
        ? new Date((rowData.date_hour - 25569) * 86400 * 1000).toISOString()
        : undefined;

      const data = new MetalurgiaCalibracionModel(rowData);
      await data.save();
    } catch (error) {
      console.error("Error al guardar el dato:", error);
    }
...
const DeleteCalibracionMetalurgia = async (req, res) => {
  console.log("Borrando datos de calibración de metalurgia");

  try {
    const response = await MetalurgiaCalibracionModel.deleteMany({});
    res.status(200).json({ Message: "Ok" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ Message: error });
...
module.exports = {
  ...
  CargarDatosMetalurgia,
  DeleteInputsMetalurgia,
  CargarDatosCalibracionMetalurgia,
  DeleteCalibracionMetalurgia
};
